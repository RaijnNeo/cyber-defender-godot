...
const SAVE_KEY  = "user://hint_floor2.cfg"
const PANEL_W   = 420
const PANEL_H   = 340
const BORDER_W  = 4
...
func _build_pages():
	...
	pages.append({
		...
		"lines": [
			...
			"HAZARD ZONE.",
			"",
			"This quarantined sector",
			"is under active malware",
			"siege. Threats here are",
			"stronger and faster.",
			"",
	...
	pages.append({
		...
		"lines": [
			"RANSOMWARE",
			"Locks your commands.",
			"Survive until it lifts.",
			"",
			"ROOTKIT",
			"50% evasion by default.",
			"Use KNOWLEDGE to reveal",
			"and break its stealth.",
			"",
			"Both are resistant to",
	...
	pages.append({
		...
		"lines": [
			"Same controls as Floor 1.",
			"",
			"BREACH TILES",
			"Glowing red/orange tiles",
			"trigger encounters.",
			"",
			"Encounter rate is higher",
			"in this sector.",
			"",
			"TAP [ ? ] to reopen",
			"this guide anytime.",
	...
	pages.append({
		...
		"lines": [
			"SYSTEM HP ALERT",
			"",
			"Battles in this sector",
			"deal 65% damage to",
			"System HP instead of 50%.",
			"",
			"Return to Floor 1 to",
			"use the Repair Terminal",
			"if critically low.",
			"",
	...
	pages.append({
		...
		"lines": [
			...
			"!! HONEYPOT WARNING !!",
			"",
			"One terminal is FAKE.",
			"Interacting with it",
			"triggers an ambush.",
			"",
			"If the label glitches",
			"red — do NOT touch it.",
		]
	})
...
func _process(delta):
	...
	var in_overworld  = current_scene != null and current_scene.scene_file_path == "res://Floor_2_Scenes/floor_2.tscn"
